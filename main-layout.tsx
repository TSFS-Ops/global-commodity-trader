...
import TradingMenu from "../nav/TradingMenu";
...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  // Dynamic trading items based on taxonomy
  ...
  const tradingItems = useMemo(() => {
    const baseItems = [
      { name: "All Listings", path: "/listings", icon: <Search size={20} /> },
    ];

    if (taxonomyData?.taxonomy?.categories) {
      // Add first 6 categories as direct menu items
      const categoryItems = taxonomyData.taxonomy.categories.slice(0, 6).map((cat: any) => ({
        name: cat.label,
        path: `/listings?category_code=${cat.code}`,
        icon: <Leaf size={20} />
      }));
      baseItems.push(...categoryItems);

      // If more than 6 categories, add "More..." link
      if (taxonomyData.taxonomy.categories.length > 6) {
        baseItems.push({
    ...
    baseItems.push(
      { name: "Buy Signals", path: "/buy-signals", icon: <RefreshCw size={20} /> },
        ...
        <div className="container mx-auto px-6 max-w-[1200px]">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-6">
              ...
              {/* Trading Menu - Dynamic from Taxonomy */}
              <TradingMenu />
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center space-x-1 text-white hover:text-[#d1e891] transition-colors cursor-pointer">
                  <Zap size={20} />
                  <span className="ml-1">Map & Blockchain</span>
                  <ChevronDown size={16} />
                </DropdownMenuTrigger>
                ...
                <SheetContent className="bg-[#173c1e] text-white">
                  <div className="py-4">
                    ...
                    <div className="space-y-1">
                      ...
                      {/* Trading Group */}
                      <div className="pt-2">
                        <p className="text-xs font-medium text-[#d1e891]/80 uppercase tracking-wide px-3 pb-1">Trading</p>
                      ...
                      {/* Tools Group */}
                      <div className="pt-2">
                        <p className="text-xs font-medium text-[#d1e891]/80 uppercase tracking-wide px-3 pb-1">Tools</p>
                        {toolsItems.map((item) => (
                          <Link
                            key={item.path}
                            to={item.path}
                      ...
                      {(user?.role === 'seller' || user?.role === 'admin') && (
                        <div className="pt-2">
                          <p className="text-xs font-medium text-[#d1e891]/80 uppercase tracking-wide px-3 pb-1">Actions</p>
                          {(user?.role === 'seller' || user?.role === 'admin') && (
                            <Link
                              to="/listings/create"
                              className={`w-full text-left flex items-center py-2 px-3 rounded-md ${
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 bg-[#173c1e] border-[#a8c566]/30 text-white">
                  ...
                  <DropdownMenuItem asChild>
                    <Link 
                      to="/profile"
                      className="flex items-center focus:bg-white/10 focus:text-[#d1e891] cursor-pointer"
                    >
